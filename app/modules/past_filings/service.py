    ...
    with logger.contextualize(domain="Filing"):
        ...
        for ticker, info in existing_companies_dict.items():
            ...
            with SessionLocal() as db:
                ...
                existing_accessions = repository.get_existing_filing_accessions(db, company_id)
                ...
                if original_count >= 20:
                    logger.info(f"[{ticker}] DB에 최근 20개 원본 공시가 이미 존재하여 외부 조회를 건너뜁니다")
                    continue
                ...
                try:
                    ...
                    logger.exception(f"[{ticker}] DB 적재 실패로 전체 롤백: {e}")
